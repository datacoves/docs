...
class DocsifyMarkdownExtension(Extension):
    ...
    def extendMarkdown(self, md):
        md.parser.blockprocessors.register(
            BlockQuoteWithAttributes(md.parser),
            'quote',
            100
        )
        ...
        with open(f"{md_file}", "rt") as input:
            return markdown.markdown(
                ...
                extensions=[
                    ...
                    CodeHiliteExtension(
                        pygments_formatter=DocsifyCodeCustomFormatter
                    ),
                ]
            )
    ...
    def render(self, md_file : str) -> str :
        ...
        for link in html.find_all('a'):
            ...
            if 'href' in link.attrs and \
               not link['href'].startswith("http://") and \
               not link['href'].startswith("https://") and \
        ...
        for parent_div in html.find_all('div', class_='codehilite'):
            ...
            inner_pre['data-lang'] = [inner_code.attrs.get('class', ['language-'])[0][9:]]
...
def main():
    if len(sys.argv) != 3:
        print(f"Syntax: {sys.argv[0]} input-dir output-dir")
        sys.exit(1)
