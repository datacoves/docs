...
func (r *WorkspaceReconciler) neo4j(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-neo4j"
	...
	if !workspace.InternalServiceEnabled("neo4j") {
		if releaseNotFound {
			return nil
		}
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["neo4j"]["repo"],
			RepoName:   workspace.Spec.Charts["neo4j"]["repo_name"],
			Chart:      workspace.Spec.Charts["neo4j"]["chart"],
			Version:    workspace.Spec.Charts["neo4j"]["version"],
			ValuesName: workspace.Spec.Configs["neo4j-values"],
		},
	}
