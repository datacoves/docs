...
func (r *WorkspaceReconciler) datahub(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-datahub"
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["datahub"]["repo"],
			RepoName:   workspace.Spec.Charts["datahub"]["repo_name"],
			Chart:      workspace.Spec.Charts["datahub"]["chart"],
			Version:    workspace.Spec.Charts["datahub"]["version"],
			ValuesName: workspace.Spec.Configs["datahub-values"],
		},
	}
