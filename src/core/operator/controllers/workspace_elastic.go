...
func (r *WorkspaceReconciler) elastic(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-elastic"
	...
	if !workspace.InternalServiceEnabled("elastic") {
		if releaseNotFound {
			return nil
		}
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["elastic"]["repo"],
			RepoName:   workspace.Spec.Charts["elastic"]["repo_name"],
			Chart:      workspace.Spec.Charts["elastic"]["chart"],
			Version:    workspace.Spec.Charts["elastic"]["version"],
			ValuesName: workspace.Spec.Configs["elastic-values"],
		},
	}
