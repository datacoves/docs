...
func (r *WorkspaceReconciler) postgresql(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-postgresql"
	...
	if !workspace.InternalServiceEnabled("postgresql") {
		if releaseNotFound {
			return nil
		}
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["postgresql"]["repo"],
			RepoName:   workspace.Spec.Charts["postgresql"]["repo_name"],
			Chart:      workspace.Spec.Charts["postgresql"]["chart"],
			Version:    workspace.Spec.Charts["postgresql"]["version"],
			ValuesName: workspace.Spec.Configs["postgresql-values"],
		},
	}
