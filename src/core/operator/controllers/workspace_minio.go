...
func (r *WorkspaceReconciler) minio(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-minio"
	...
	if !workspace.InternalServiceEnabled("minio") {
		if releaseNotFound {
			return nil
		}
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["minio"]["repo"],
			RepoName:   workspace.Spec.Charts["minio"]["repo_name"],
			Chart:      workspace.Spec.Charts["minio"]["chart"],
			Version:    workspace.Spec.Charts["minio"]["version"],
			ValuesName: workspace.Spec.Configs["minio-values"],
		},
	}
