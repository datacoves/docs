...
func (r *WorkspaceReconciler) superset(ctx context.Context, workspace *Workspace) error {
	log := log.FromContext(ctx)
	ns := workspace.Namespace
	releaseName := workspace.Name + "-superset"
	...
	release := &HelmRelease{
		...
		Spec: HelmReleaseSpec{
			RepoURL:    workspace.Spec.Charts["superset"]["repo"],
			RepoName:   workspace.Spec.Charts["superset"]["repo_name"],
			Chart:      workspace.Spec.Charts["superset"]["chart"],
			Version:    workspace.Spec.Charts["superset"]["version"],
			ValuesName: workspace.Spec.Configs["superset-values"],
		},
	}
