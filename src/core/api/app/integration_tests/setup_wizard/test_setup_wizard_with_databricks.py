...
data = {
    ...
    "host": "adb-1158734404438996.16.azuredatabricks.net",
    "http_path": "/sql/1.0/warehouses/c66bb3ad1820877e",
    "schema": "default",
    "token": utils_test.secrets["databricks_service_account"]["token"],
    "git_repo_ssh_url": "git@github.com:datacoves/balboa.git",
    "release_branch": "main",
}


class SetupWizardWithDatabricksTest(SetupWizardBase):
    def setUp(self):
        self._subpath = f"setup_wizard/{__name__}"
        return super().setUp()

    @pytest.mark.asyncio
    async def test_setup_wizard_with_databricks(self):
        """Test to setup an account with Databricks"""
        ...
        try:
            ...
            print("Setup wizard completed with Databricks completed")
    ...
    async def setup_wizard_step_three(self):
        """Complete the step three with Redshift warehouse"""

        await asyncio.sleep(2)  # Wait until the accordion is fully open.
        await self.page.locator("#type").select_option(data["conn_type"])
        await self.page.get_by_label("Host").fill(data["host"])
        await self.page.get_by_label("HTTP Path").fill(data["http_path"])
        await self.page.get_by_label("Schema").fill(data["schema"])
        await self.page.get_by_label("Token").fill(data["token"])
