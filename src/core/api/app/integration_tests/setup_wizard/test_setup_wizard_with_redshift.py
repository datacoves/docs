...
data = {
    ...
    "conn_type": "redshift",
    "host": "default.468004607141.us-west-2.redshift-serverless.amazonaws.com",
    "database": "dev",
    "schema": "sebastian",
    "user": utils_test.secrets["redshift_service_account"]["user"],
    "password": utils_test.secrets["redshift_service_account"]["password"],
    "git_repo_ssh_url": "git@github.com:datacoves/balboa.git",
    "release_branch": "main",
}


class SetupWizardWithRedshiftTest(SetupWizardBase):
    def setUp(self):
        self._subpath = f"setup_wizard/{__name__}"
        return super().setUp()

    @pytest.mark.asyncio
    async def test_setup_wizard_with_redshift(self):
        """Test to setup an account with Redshift"""
        ...
        try:
            ...
            print("Setup wizard completed with Redshift completed")
