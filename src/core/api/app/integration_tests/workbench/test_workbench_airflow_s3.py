...
class WorkbenchAirflowWithS3Test(WorkbenchBase):
    def setUp(self):
        self._subpath = f"workbench/{__name__}"
        return super().setUp()

    @pytest.mark.skip(
        reason="No way of currently testing this, it's always failing on Github Action"
    )
    @pytest.mark.asyncio
    async def test_workbench_airflow_with_s3(self):
        """
        ...
        try:
            ...
            print("Workbench Airflow with S3 completed")
    ...
    async def configure_airflow_stack(self):
        ...
        await self.page.page.get_by_label("Bucket path*").fill(
            utils_test.secrets["airflow_s3_sync"]["bucket"]
        )
        await self.page.get_by_role("combobox", name="Auth mechanism").select_option(
            "iam-user"
        )
        await self.page.get_by_label("Access key*").fill(
            utils_test.secrets["airflow_s3_sync"]["access_key"]
        )
        await self.page.get_by_label("Secret key*").fill(
            utils_test.secrets["airflow_s3_sync"]["secret_key"]
        )
