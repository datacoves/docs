...
import { useCreateProfileSSHKeys } from '../api/createProfileSSHKeys';

// prettier-ignore
export const ProfileAddSSHKeysMenu = ({ handleGenerate }: { handleGenerate: (keyType?: string) => void }) => {
  const { isOpen, onOpen, onClose } = useDisclosure();

  const createMutation = useCreateProfileSSHKeys(onClose);
  ...
  return (
    <>
      <MenuList>
        ...
        <MenuItem onClick={() => handleGenerate('rsa')}>Auto-generate RSA key pairs (Compatibility with Azure)</MenuItem>
        <MenuItem onClick={onOpen}>Provide private key</MenuItem>
      </MenuList>
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Provide private SSH key</ModalHeader>
          <ModalCloseButton />
          <Formik
