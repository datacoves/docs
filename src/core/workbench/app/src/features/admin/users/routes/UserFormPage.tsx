...
import { useUser } from '../api/getUserById';
...
export const UserFormPage = () => {
  ...
  const handleSubmit = (body: any, { setSubmitting }: any) => {
    ...
    if (!isCreateMode && currentAccount?.slug !== undefined) {
      updateMutation.mutate({ account: currentAccount?.slug, id, body });
    }
    setSubmitting(false);
  };
  ...
  return (
    <BasePage header={isCreateMode ? 'Create user' : 'Edit user'}>
        ...
        <Stack spacing="4" divider={<StackDivider />}>
          ...
          >
            {function Render({ setFieldValue, values }) {
              const { data, isSuccess, isLoading } = useUser(currentAccount?.slug, id, {
