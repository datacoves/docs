    ...
    if directory.stat().st_uid == os.getuid():
        if directory.stat().st_mode % 0o1000 != folder_permissions % 0o1000:
            ...
            try:
                ...
                print(
                    f"Failed to change {directory} permission to {folder_permissions}: {e}"
                )
                return
...
class FileTaskHandler(logging.Handler):
    ...
    def __init__(self, base_log_folder: str, filename_template: str | None = None):
        ...
        if filename_template is not None:
            warnings.warn(
                ...
                stacklevel=(2 if type(self) == FileTaskHandler else 3),  # noqa
            )
    ...
    def _render_filename(
        self, ti: TaskInstance | TaskInstanceKey, try_number: int
    ) -> str:
    ...
    def _init_file(self, ti, *, identifier: str | None = None):
        ...
        if not os.path.exists(full_path):
            ...
            # HOTFIX: Catch PermissionError exception to avoid issues in Azure Blob Storage
            try:
                os.chmod(full_path, new_file_permissions)
            except (OSError, PermissionError) as e:
                # Avoid printing warning in task logs
                # logging.warning("OSError while changing ownership of the log file. ", e)
                logging.debug("OSError while changing ownership of the log file. ", e)
    ...
    def _read_from_logs_server(
        ...
        try:
            ...
            if (
                isinstance(e, UnsupportedProtocol)
