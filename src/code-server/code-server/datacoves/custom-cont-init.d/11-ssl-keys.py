...
def generate_keys():
    ...
    output_folder = Path("/config/.ssl")
    keys_file = Path("/opt/datacoves/user/ssl_keys.json")
    ...
    if keys_file.exists():
        ...
        for key in data:
            private_path = output_folder / f"{key['connection']}-private.pem"
            print(f"Generating {private_path}...")
            with open(private_path, "w") as private_file:
                private_file.write(key["private"])

            public_path = output_folder / f"{key['connection']}-public.pem"
