...
While case sensitive names can be used in Snowflake, they require double quotes to use and using mixed case introduces potential for duplication (fieldName and FieldName are different objects). We use snake_case for a consistent structure throughout our Snowflake databases. Similarly, all models managed by dbt must have unique names to allow references to be driven by the name of the table or view.

As a general rule, our names use full words and increase in granularity left-right: patient_address_city. This assists with the sort order of our objects, grouping models and columns into obvious sections.

## Snowflake databases

The Snowflake warehouse is separated into two primary parts:
1. The **raw** database where data gets ingested into Snowflake and which is not exposed to most users.
...
Sensitive data is restricted at the column level throughout the system: if sensitive data exists, the schema is given a suffix of _\_pii_ to give users a hint that there is sensitive data in this schema. A corresponding security role must be assigned to a developer to be able to see this data.

Data flattening is managed with dbt in the /models/inlets/\<source_name\> folder of the dbt repository. Flattened models must be created by developers permitted to see the sensitive data. These developers will flatten data to create more useable columns and they will apply masking rules.
...
This dbt-managed database contains all transformed models (inlets, bays, and coves)  prepared for use by the business. Development and Test environments exist in parallel. This is where new models are developed and checked before they are released to production.
...
The **RAW_PRD** database is primarily populated directly from vendor / source system data, and exists as a mirror in place of direct connection to those sources. Schemas are created as < system / vendor name > in Fivetran / AirByte config (Airbyte calls schemas Namespaces), and tables keep the name given by the source. Where a source is unique to a specific country, the schema should reflect that as < system / vendor name >\_< country short code > for clarity.

### Raw Database Tables

To account for schema drift, all data is loaded into VARIANT (semi-structured) columns directly in the database. We create < schema name >\_< source table name >[\_< nested object key >] tables, with columns matching the keys of the original source.
...
An AirByte source configuration should be given the name of the source itself allowing clear visibility of where the data comes from without the need to open a connection.
...
Any aggregation should be described in a suffix on the _model/column: customer_countries.customer_count_.
