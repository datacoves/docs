...
- Rather than developing complex queries against all data in the Development environment, consider adding a where condition to run just the most recent data - it will speed up query time, and with a {% if target.name == 'dev' %} statement can be easily removed for testing and production.
...
- All models should be initially created as views or ephemeral; moving to tables / incremental tables where required for performance.

### Model attribute layout

- Any id columns should be listed first in a model (model key first, followed by any foreign keys

- Any created/modified dates should be listed last in a model, followed by internal fields (AirByte etc)
